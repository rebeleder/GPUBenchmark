...
async function renderChart(options) {
    ...
    try {
        ...
        // Dynamically set chart height to unify bar width
        const chartContainer = document.getElementById(chartId);
        if (chartContainer) {
            const barHeight = 22; // Set a fixed height for each bar
            const gridTop = 100;  // Corresponds to grid.top in options
            const gridBottom = 80; // Corresponds to grid.bottom in options
            const newHeight = processedData.length * barHeight + gridTop + gridBottom;
